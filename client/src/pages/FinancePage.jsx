...
const FinancePage = () => {
  ...
  useEffect(() => {
    const fetchFinanceArticles = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/articles/category/finance');
        const data = await response.json();
        
        if (response.ok) {
          console.log('獲取財經文章成功:', data);
          if (data.data && Array.isArray(data.data)) {
            setFinanceArticles(data.data);
          } else {
            setFinanceArticles([]);
          ...
          throw new Error(data.message || '無法獲取財經文章');
        }
      } catch (err) {
        console.error('獲取財經文章出錯:', err);
  ...
  const totalPages = Math.ceil(financeArticles.length / articlesPerPage);
  ...
  // 当前页的文章
  const currentArticles = financeArticles.slice(
    ...
    <div className="finance-page">
      ...
      <div className="finance-content container">
        <div className="breadcrumb">
          ...
          <span className="current">財經</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-message">暫無財經文章</div>
        ) : (
          <ul className="finance-articles">
        ...
        {/* 分页控制 */}
        {!loading && !error && financeArticles.length > 0 && (
