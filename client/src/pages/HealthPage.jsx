...
const HealthPage = () => {
  ...
  useEffect(() => {
    const fetchHealthArticles = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/articles/category/health');
        const data = await response.json();
        
        if (response.ok) {
          console.log('獲取健康文章成功:', data);
          ...
          throw new Error(data.message || '無法獲取健康文章');
        }
      } catch (err) {
        console.error('獲取健康文章出錯:', err);
  ...
  const totalPages = Math.ceil(healthArticles.length / articlesPerPage);
  ...
  // 当前页的文章
  const currentArticles = healthArticles.slice(
    ...
    <div className="health-page">
      ...
      <div className="health-content container">
        <div className="breadcrumb">
          ...
          <span className="current">健康</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-message">暫無健康文章</div>
        ) : (
          <ul className="health-articles">
        ...
        {/* 分页控制 */}
        {!loading && !error && healthArticles.length > 0 && (
